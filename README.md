...
3. how to preprocess with your data
   - inpaint: run `generate_inpaint_img.py` and specify the `input_dir`, `mask_dir`, and `output_dir`.
   - saliency detection: run `saliency_detection.py` and specify the `WEIGHT_ROOT`.
   - get saliency detection box: run `generate_sal_box.py` and specify the `input_dir`,  and `output_dir`.
