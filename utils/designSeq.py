...
def sort_boxes_by_area(boxes):
    ...
    sorted_indices = torch.argsort(torch.tensor(areas), descending=True)
    sorted_boxes = boxes[sorted_indices]
    return sorted_boxes

def reorder_pku(cls, box, max_elem=None):
...
def reorder_cgl(cls, box, max_elem=None):
    if max_elem is None:
        max_elem = len(cls)
    # init
    order = []
    ...
    logo = np.where(cls == 2)[0]
    deco = np.where(cls == 3)[0]
    embl = np.where(cls == 4)[0]  # 添加embl类别
    order_text = sorted(np.array(list(enumerate(area)))[text].tolist(), key=lambda x: x[1], reverse=True)
    order_deco = sorted(np.array(list(enumerate(area)))[deco].tolist(), key=lambda x: x[1])
